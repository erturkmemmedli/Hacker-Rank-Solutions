...
class MinimumOperations {
    ...
    public void minOperations() {
        int i, j;
        for (i = 0; i <= n; i++) { // change 1
            for (j = 0; j < 8; j++) { // change 2
                dp[i][j] = (1<<30);
            }
        }
        dp[0][0] = 0;
        for (i = 0; i < n; i++){
            for (j = 0; j < 8; j++){ // change 3
                dp[i + 1][j | 1] = Math.min(dp[i + 1][j | 1], dp[i][j] + green.get(i) + blue.get(i));
                dp[i + 1][j | 2] = Math.min(dp[i + 1][j | 2], dp[i][j] + red.get(i) + blue.get(i));
                dp[i + 1][j | 4] = Math.min(dp[i + 1][j | 4], dp[i][j] + red.get(i) + green.get(i)); // change 4
            }
        }
        j = 0;
        for (i = 0; i < n; i++){
            if (red.get(i) != 0) j |= 1; // change 5
            if (green.get(i) != 0) j |= 2; // change 6
            if (blue.get(i) != 0) j |= 4;
        }
